<script lang="ts">
	...
	onMount(async () => {
		if (!$appSession.tokens.bitbucket) {
			await getBitbucketToken();
		}
		loading.base = true;
		try {
			const workspaces = await get(`${apiUrl}/2.0/user/permissions/workspaces`, {
				Authorization: `Bearer ${$appSession.tokens.bitbucket}`
			});
			if (workspaces.size > 0) {
				workspaces.values.forEach((workspace: any) => {
					const { name, slug } = workspace.workspace;
	...
	async function getBitbucketToken() {
		return await new Promise<void>((resolve, reject) => {
			...
			const newWindow = open(
				`${htmlUrl}/site/oauth2/authorize?client_id=${application.gitSource.bitbucketApp.consumerKey}&response_type=token`,
				'Bitbucket',
			...
			const timer = setInterval(() => {
				if (newWindow?.closed) {
					clearInterval(timer);
					$appSession.tokens.bitbucket = localStorage.getItem('bitbucketToken');
					resolve();
				}
			}, 100);
		});
	...
	async function loadProjects(nextPage: string|null = null) {
		...
		try {
			let newProjects;
			if (nextPage) {
				newProjects = await get(nextPage, {
				Authorization: `Bearer ${$appSession.tokens.bitbucket}`
			});
			} else {
				newProjects = await get(`${apiUrl}/2.0/repositories/${selected.group.value}`, {
				Authorization: `Bearer ${$appSession.tokens.bitbucket}`
			});
			}
			projects = projects.concat(newProjects.values);
			if (newProjects.next) {
				await loadProjects(newProjects.next);
			}
		} catch (error) {
			return errorNotification(error);
		} finally {
