...
export async function listSources(request: FastifyRequest) {
	try {
		...
		const sources = await prisma.gitSource.findMany({
			...
			include: { teams: true, githubApp: true, gitlabApp: true, bitbucketApp: true }
		});
		return {
			sources
		};
...
export async function getSource(request: FastifyRequest<OnlyId>) {
	try {
		...
		const source = await prisma.gitSource.findFirst({
			...
			include: { githubApp: true, gitlabApp: true, bitbucketApp: true }
		});
		if (!source) {
...
export async function saveBitbucketSource(request: FastifyRequest<SaveBitbucketSource>) {
	try {
		const { id } = request.params;
		const { teamId } = request.user;
		let {
		...
		if (id === 'new') {
			...
			await prisma.bitbucketApp.create({
				data: {
					teams: { connect: { id: teamId } },
					consumerKey,
					consumerWorkspaceName,
			...
			await prisma.bitbucketApp.update({
				where: { id },
				data: {
					consumerKey,
					consumerWorkspaceName,
