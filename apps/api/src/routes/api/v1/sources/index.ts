...
import type { CheckGitLabOAuthId, SaveBitbucketSource, SaveGitHubSource, SaveGitLabSource } from './types';
...
const root: FastifyPluginAsync = async (fastify): Promise<void> => {
	...
	fastify.post<CheckGitLabOAuthId>(
		'/:id/check',
		async (request) => await checkGitLabOAuthID(request)
	);
	...
	fastify.post<SaveBitbucketSource>('/:id/bitbucket', async (request) => await saveBitbucketSource(request));
};
